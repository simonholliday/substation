...
# Target Intermediate Frequency (IF) sample rate for NFM demodulation
# Decimating to near this rate before NFM discriminator improves SNR.
# We pick the closest integer decimation factor to this target.
TARGET_IF_SAMPLE_RATE = 48000
...
def demodulate_nfm (
	...
	# We want if_rate to be >= 2 * bandwidth (approx 48kHz is safe for 12.5kHz NFM).
	if_decimation = max(1, int(round(sample_rate / TARGET_IF_SAMPLE_RATE)))
	...
	# 2. FM demodulation: instantaneous frequency is the phase difference per sample.
	...
	# 3. De-emphasis compensates for transmitter pre-emphasis.
	...
	# 5. Normalize based on deviation and sample rate.
	...
	# 6. Decimate to final audio rate.
