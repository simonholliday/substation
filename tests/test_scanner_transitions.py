...
class TestRefineTrimOnAudio:
	...
	def test_turn_on_trims_silence (self):
		...
		trimmed, pad = substation.scanner.RadioScanner._refine_trim_on_audio(audio, turning_on=True)
		# Should have trimmed some leading silence
		assert len(trimmed) < len(audio)
		# Pad should be at most TRIM_PRE_SAMPLES
		assert pad <= substation.constants.TRIM_PRE_SAMPLES
		assert pad > 0
	...
	def test_turn_off_trims_trailing (self):
		...
		trimmed, pad = substation.scanner.RadioScanner._refine_trim_on_audio(audio, turning_on=False)
		assert len(trimmed) < len(audio)
		assert pad <= substation.constants.TRIM_POST_SAMPLES
	...
	def test_all_signal (self):
		...
		trimmed_on, pad_on = substation.scanner.RadioScanner._refine_trim_on_audio(audio, turning_on=True)
